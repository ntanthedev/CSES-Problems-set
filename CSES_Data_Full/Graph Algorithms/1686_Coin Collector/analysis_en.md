...
We first identify the strongly connected components and create a component graph
where each node's value is the total number of coins in the component.
We use Kosaraju's algorithm for finding the components.

After that, we find the maximum coin sum in a path in the component graph.
We can use dynamic programming to find the maximum sum
because the component graph is a directed acyclic graph.
In the dynamic programming implementation, we use the fact that
Kosaraju's algorithm creates the components in reverse topological order.

The resulting algorithm works in $O(n+m)$ time.
