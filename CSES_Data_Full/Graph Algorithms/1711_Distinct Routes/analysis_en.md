...
The maximum number of distinct paths equals the maximum flow in the graph. The following solution uses the scaling algorithm to find the maximum flow.

We also need to identify the distinct paths. We can find each path using a greedy algorithm that finds a path from the source to the sink where every edge has flow. After finding a path, we remove the flow from the path edges.
