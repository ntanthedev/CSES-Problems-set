...
The problem can be modeled as a 2SAT problem, where each topping is assigned a variable
and each wish corresponds to a clause involving two variables.

The following code solves the 2SAT problem by constructing a graph in which each node
represents either a variable or its negation.
Each clause creates two edges that define the relations between the variables.
The problem has a solution exactly when there is no strongly connected component
that contains both a variable and its negation.
If a solution exists, the values for the variables can be assigned
according to the topological sort order.
...
* [CPHB (Competitive Programmer's Handbook)](http://cses.fi/book), Chapter 17