...
We create a segment tree where each node stores four values:

* `total_sum`: the total sum of values in the range
* `max_left`: the maximum sum starting from the left end of the range
* `max_right`: the maximum sum ending at the right end of the range
* `max_sum`: the maximum subarray sum in the range

Using these four values, we can efficiently calculate the maximum subarray sum for any given range.

The solution works in $O((n+q) \log n)$ time. While the problem does not require it, this approach also supports update queries.
