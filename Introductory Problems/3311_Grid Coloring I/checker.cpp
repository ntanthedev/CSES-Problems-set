 ...
 *    Input read:    n, m; then n grid rows (each m chars A/B/C/D) via readToken
 *    Validity:     output is IMPOSSIBLE iff jury says so; else n rows of m chars,
 *                  each in {A,B,C,D}, each cell differs from input, no equal
...
static void validateColoring(int n, int m, const vector<string>& initial,
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            char c = out[i][j];
            if (!isAllowed(c))
                quitf(_wa, "row %d col %d: invalid character '%c' (expected A/B/C/D)",
                      i + 1, j + 1, c);
            if (c == initial[i][j])
...
int main(int argc, char* argv[]) {
    ...
    if (!ouf.seekEof())
        quitf(_wa, "extra information in the output file");
    quitf(_ok, "valid 4-coloring of %dx%d grid", n, m);
}
