...
Our task is to find an Euler path in a directed graph.
There are two conditions that must hold for the graph to have an Euler path.

First, if we add a new edge from node $n$ to node $1$,
then each node's indegree and outdegree must be equal.

Second, all the edges must be in the same connected component.
