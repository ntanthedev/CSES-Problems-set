...
int main(int argc, char* argv[]) {
    ...
    if (outFirst == "IMPOSSIBLE") {
        if (ansFirst != "IMPOSSIBLE")
            quitf(_wa, "Output is IMPOSSIBLE but a directed cycle exists");
        if (!ouf.seekEof())
        quitf(_wa, "Extra information in the output file");
        quitf(_ok, "Correct: no directed cycle");
    }

    if (ansFirst == "IMPOSSIBLE")
        quitf(_wa, "A directed cycle exists but output is IMPOSSIBLE");

    int k = stoi(outFirst);
    if (k < 2)
        quitf(_wa, "Cycle must have at least 2 cities (start repeated), got %d", k);
    ...
    for (int i = 0; i + 1 < k; i++) {
        ...
        if (!found)
            quitf(_wa, "No directed flight from %d to %d in cycle", u, v);
    }
